...
**/
template<class ItemType>
void ArrayBag<ItemType>::operator/=(const ArrayBag<ItemType>& rightbag){
    for (int i = 0; i < rightbag.item_count_; i++){
        if (!contains(rightbag.items_[i])){
            add(rightbag.items_[i]);
        }
    }
}
/**
...
// Assuming the definition of ArrayBag and its private members.
