...
/**
  ...
  @return     : Returns true if the right hand side character is "equal", false otherwise.                      
                Two characters are equal if they have the same name, same race, same level, and are either both 
                an enemy or not.
                NOTE: By this definition, only the aforementioned subset of the character's attributes must be 
                equal for two characters to be deemed "equal".
*/
bool Character::operator==(const Character& right) const{
    if(name_ == right.name_ && race_ == right.race_ && level_ == right.level_ && enemy_ == right.enemy_){
        return true;
    }
    return false;
}
/**
  @param      : A const reference to the right hand side of the != operator.
  @return     : Returns true if the right hand side character is NOT "equal" (!=), false otherwise. 
                Two characters are NOT equal if any of their name, race, or level are not equal, or if one 
                is an enemy and the other is not.
                NOTE: By this definition, one or more of the aforementioned subset of the character's attributes 
                only must be different for two characters to be deemed "NOT equal".
*/
bool Character::operator!=(const Character& right) const{
    if(name_ != right.name_ || race_ != right.race_ || level_ != right.level_ || enemy_ != right.enemy_){
        return true;
