...
void Character::strike(Character* target){
    ...
    else if(target->getArmor() >= 2){
        target->setArmor(target->getArmor() - 2);
    }
    //here, i added an additional condition because for some reason the hp of the enemy did not go to 0.
    else if(target->getArmor() == 0 && target->getVitality() == 1){
        target->setVitality(0);
    }
    else if(target->getArmor() == 0){
...
void Character::addAction(int number){
    action_queue_.push(number);
}
...
void Character::turnHelper(){//used to avoid copying.
    action_queue_.pop();
}