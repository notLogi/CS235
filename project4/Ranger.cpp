...
void Ranger::display() const{
    ...
    std::string animalTrue = getCompanion() ? "TRUE" : "FALSE";

    std::cout << getName() << " is a Level " << getLevel() << " " << getRace() << " RANGER." << "\nVitality: " << getVitality() << "\nArmor: " << getArmor() << "\nThey are " << enemyTrue << "\nAnimal Companion: " << animalTrue << 
    "\nArrows: \n";
    for(int i = 0; i < getArrows().size(); i++){
        std::cout << getArrows()[i].type_ << ": " << getArrows()[i].quantity_ << std::endl;
    }
  std::cout << "Affinities: \n";
...
void Ranger::eatTaintedStew(){
    if(getRace() == "UNDEAD"){
        ...
        for (const std::string& affinity : getAffinities()) {
            if (affinity == "POISON") {
        ...
        if (getCompanion() && getRace() != "UNDEAD") {
